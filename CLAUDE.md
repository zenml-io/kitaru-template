...
- `returns_agent/fixtures.py` — `ORDERS`, `POLICIES`, `SHIPMENTS`, and the ten `CASES` ticket inputs. Tickets carry no outcome labels; expected outcomes live only in test-side tables (`tests/canonical_returns_agent.py`, `tests/canonical_returns_evaluator.py`).
- `tests/canonical_returns_agent.py` — a provider-free "candidate v2" agent used by the e2e test: it replays scripted tool calls through a `FunctionModel` (no LLM), letting the experiment/replay loop run in CI without API keys.
- The e2e test walks the full documented product loop: register agent → start worker → import traces → evaluate → create investigation with annotations/verdicts → register a custom evaluator → build cohorts → register candidate version → run experiments → verify replayed sessions pass the policy evaluator.
